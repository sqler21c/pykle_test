...
class MainWindow(wx.Frame,InetFTP.InetFTP):
    def _init_ctrls(self, prnt):
        ...
        wx.Frame.__init__(self, id=wxID_MAINWINDOW, name=u'MainWindow',
              parent=prnt, pos=wx.Point(33, 120), size=wx.Size(1297, 584),
              style=wx.NO_FULL_REPAINT_ON_RESIZE | wx.FRAME_TOOL_WINDOW | wx.DEFAULT_FRAME_STYLE,
              title=const.TOOL_VERSION)
        self.SetClientSize(wx.Size(1289, 560))
        ...
        self.ToolClose.SetAutoLayout(True)
        ...
        self.LogView = wx.TextCtrl(id=wxID_MAINWINDOWLOGVIEW, name=u'LogView',
              ...
              style=wx.WANTS_CHARS | wx.TE_LINEWRAP | wx.TE_WORDWRAP | wx.TE_MULTILINE | wx.HSCROLL | wx.VSCROLL | wx.TE_READONLY,
              value=u'')
        self.LogView.SetToolTipString(u'LogView')
        self.LogView.SetEditable(False)
        self.LogView.Enable(True)

    def __init__(self, parent):
        self._init_ctrls(parent)

        tempTime = time.localtime(time.time())
        srtprelog = "%d%d%d%d%d%d" %(tempTime.tm_year,tempTime.tm_mon,tempTime.tm_mday,
                                    tempTime.tm_hour,tempTime.tm_min,tempTime.tm_sec)
        try:
            self.oFile = open(srtprelog+".log", "a")
        except IOError:
    ...
    def OnConDisButton(self, event):
        ...
        while n <= int(counter):
            ...
            if srvstate == const.SRV_STATE_IDLE:
                vbool = self.SrvStateChange(srvstate)
                if not vbool : 
                    self.WriteLog("server state change  : fail")
                    return False
            elif srvstate == const.SRV_STATE_CONNECTED or srvstate == const.SRV_STATE_DORMANT :
                self.vbool = self.SrvStateChange(srvstate)
                if not self.vbool : 
                    self.WriteLog("server state change  : fail")
                    return False
            elif srvstate == const.SRV_STATE_INACTIVE:
                self.WriteLog("Server state :  %s device Check" %srvstate)
                return False
            elif srvstate == const.SRV_STATE_SEARCHING:
                self.WriteLog("Server state :  %s device Check" %srvstate)
                return False
            elif srvstate == const.SRV_STATE_UNAVAILABLE:
                self.WriteLog("Server state :  %s device Check" %srvstate)
                return False
            elif srvstate == const.SRV_STATE_UPDATING:
                self.WriteLog("Server state :  %s device Check" %srvstate)
                return False
    ...
    def WriteLog(self,logString):
        prelog = self.SetPreLog()
        setlogstring = prelog + logString + "\n"
        self.LogView.write(setlogstring)
    ...
    def SetPreLog(self):
        tempTime = time.localtime(time.time())
        prelog = "%d%d%d%d%d%d   " %(tempTime.tm_year,tempTime.tm_mon,tempTime.tm_mday,
                                    tempTime.tm_hour,tempTime.tm_min,tempTime.tm_sec)
        return prelog 
...
###
        ...
        readsource = self.ConnectionHTTP(testtype)
        domtree = ElementTree(XML(readsource[2]))
        domtree.getroot()
    ...
    def GetSrvID(self,testtype=const.TEST_TYPE_CONDIS):
        ...
        #consource = urllib.urlopen(const.SERVER_URL+testtype)
        #readsource = self.consource.read()
        readsource = self.ConnectionHTTP(testtype)
      ...
      #  self.WriteLog(self.readsource)
        ...
        domtree.getroot()
        emel = domtree.findtext("p-answer/id")
        self.WriteLog(const.STR_SHAP)
        self.WriteLog("Server ID : " + emel)
        self.WriteLog(const.STR_SHAP)
        #consource.close()
    ...
    def ActSrvURL(self,actiontype,testtype=const.TEST_TYPE_CONDIS, phnumber="",smsbody=""):
        ...
        if testtype == const.TEST_TYPE_CONDIS:
            srvAction =  const.TEST_TYPE_CONDIS + "?id=%s&action=%s" %(srvid,actiontype)
        elif testtype == const.TEST_TYPE_MSG:
            #end url : 192.168.7.2:4330/messaging?action=send& amp;serviceid=0&to=&body=message body
            srvAction = const.TEST_TYPE_MSG + "?action="+ actiontype +"&servvice=0&to"+ phnumber + "&body=" + smsbody
    ...
    def SrvStateChange(self,srvState):
        ...
        readsource = self.ConnectionHTTP(testtype=ActURL)
        srvstate = self.GetSRVConnState()
    ...
    def OnSmsTestButton(self, event):
        srvState = self.GetSRVConnState()
        counter = self.smsCounter.GetValue()
    ...
    def GetEngData(self):
        ...
        readsource = self.ConnectionHTTP(testtype=const.TEST_TYPE_ENGDATA)
        domtree = ElementTree(XML(readsource[2]))
        domtree.getroot()
    ...
    def ConnectionHTTP(self,testtype=const.TEST_TYPE_CONDIS):
        conn = httplib.HTTPConnection(const.SERVER_URL1)
        try:
            conn.request("GET", testtype)
            response = conn.getresponse()
        except httplib.HTTPException:
            wx.MessageBox("httpconnection error check plz device")
            self.WriteLog(httplib.HTTPException)
